...
use xqpath::{
    extractor::extract,
    parse_path, parse_path_expression, evaluate_path_expression,
};
...
impl ExtendedExtractor {
    ...
    pub fn extract_auto(
        ...
        if path_or_expression.contains('|') || path_or_expression.contains(',') {
            ...
            // 转换 &Value 到 Value
            let owned_results: Vec<Value> = results.into_iter().cloned().collect();
            Ok(owned_results)
        }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 演示 2: 新的管道语法
    println!("2. 管道操作 - 员工姓名:");
    let result2 = ExtendedExtractor::extract_auto(".company.employees | [*] | .name", &data)?;
    println!("   结果: {:?}\n", result2);
    ...
    for expr in test_expressions {
        ...
        println!("   '{}' -> {}", expr,
            if has_operators { "新表达式语法" } else { "传统路径语法" });
    }
